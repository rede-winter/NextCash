...
import com.nextplugins.cash.ranking.RankingLocations;
...
public final class CashCommand {
    ...
    public void cashCommand(Context<CommandSender> context, @Optional OfflinePlayer target) {
        ...
        if (target == null) {
            ...
            sender.sendMessage(
                    MessageValue.get(MessageValue::seeBalance).replace("$amount", NumberUtil.format(balance)));
            return;
    ...
    public void cashResetCommand(Context<CommandSender> context, OfflinePlayer target) {
        ...
        if (target != null) {
            ...
            sender.sendMessage(
                    MessageValue.get(MessageValue::resetBalance).replace("$player", targetAccount.getOwner()));
        } else {
            sender.sendMessage(MessageValue.get(MessageValue::invalidTarget));
        }
    }
    ...
    public void npcAddCommand(Context<Player> context, int position) throws IOException {
        ...
        if (locations.getLocation(position) != null) {
            player.sendMessage(MessageValue.get(MessageValue::positionAlreadyDefined));
            return;
        }

        locations.setLocation(position, player.getLocation().add(0.5, 0, 0.5));
    ...
    public void npcRemoveCommand(Context<Player> context, int position) throws IOException {
        ...
        locations.setLocation(position, null);
    ...
    public void createCheckCommand(Context<Player> context, double amount, @Optional Player target) {
        ...
        if (amount < minValue) {
            player.sendMessage(
                    MessageValue.get(MessageValue::checkMinimumValue).replace("$amount", NumberUtil.format(minValue)));
            return;
        }
        ...
        player.sendMessage(
                MessageValue.get(MessageValue::checkCreated).replace("$checkValue", NumberUtil.format(amount)));
